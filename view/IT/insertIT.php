...
<script>
    $(function() {
        ...
        // Stock
            ...
            // Populate stores
            $.getJSON("https://sae401thibault.alwaysdata.net/api/api.php?actionGet=stores", function(data) {
                data.forEach(store => {
        ...
        // Employee
            ...
            formHtml += createInput("Employee Password", "employee_password", "password", true, "6 character min");
            formHtml += `
                <div class="mb-3">
            ...
            $("#submit").off("click").on("click", function(e) {
                e.preventDefault();
                $.ajax({
                    url: `https://sae401thibault.alwaysdata.net/api/api.php?actionPost=insertEmployee&store=${$("#store_id").val()}&name=${$("#employee_name").val()}&email=${$("#employee_email").val()}&password=${$("#employee_password").val()}&role=${$("#employee_role").val()}&KEY=e8f1997c763`,
                    type: "POST",
                    dataType: "json",
                    success: function(response) {
                        if (response.error) {
