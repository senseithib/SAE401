...
<script>
    $(function() {
        ...
        if (add === "product") {
            ...
            formHtml += createInput("Model Year", "model_year", "number", false, "Optional");
            resetForm(formHtml);
        ...
        // Employee
            ...
            let formHtml = createSelect("Store", "store_id");
            formHtml += createInput("Employee Name", "employee_name");
            formHtml += createInput("Employee Email", "employee_email", "email");
            formHtml += createInput("Employee Password", "employee_password", "password", true);
            formHtml += `
                <div class="mb-3">
                    <label for="employee_role" class="form-label">Employee Role</label>
                    <select id="employee_role" name="employee_role" class="form-select">
            ...
            // Populate store (only one for employee)
            $.getJSON("https://sae401thibault.alwaysdata.net/api/api.php?actionGet=store&id=<?php echo $_SESSION["StoreEmployee"] ?>", function(store) {
                $("#store_id").append(`<option value="${store[0].store_id}" selected>${store[0].store_name}</option>`);
            });

            $("#submit").off("click").on("click", function(e) {
                e.preventDefault();
                $.ajax({
                    url: `https://sae401thibault.alwaysdata.net/api/api.php?actionPost=insertEmployee&store=${<?php echo $_SESSION["StoreEmployee"] ?>}&name=${$("#employee_name").val()}&email=${$("#employee_email").val()}&password=${$("#employee_password").val()}&role=employee&KEY=e8f1997c763`,
                    type: "POST",
                    dataType: "json",
                    success: function(response) {
                        if (response.error) {
    ...
    <div class="card shadow p-4 bg-light">
        <h2 class="text-center mb-4 text-dark">Insert</h2>
        <div class="insertion">
            <form method="POST" class="form row g-3">
            </form>
            <div class="d-flex justify-content-center">
                <input type="submit" value="Insert" id="submit" class="btn btn-success mt-3">
            </div>
